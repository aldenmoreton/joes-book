...
use crate::{
    objects::{BookRole, BookSubscription, Event, EventContent, Pick},
    server::{get_book, get_picks, get_spread_teams, is_open, save_picks, set_open},
};
...
pub fn Chapter() -> impl IntoView {
    ...
    let status_fetcher = create_resource(|| (), move |_| is_open(chapter_id));

    let pick_fetcher = create_resource(move || new_picks.get(), move |_| get_picks(chapter_id));
...
pub fn ChapterEvents(initial_values: Vec<(String, Vec<(Event, Pick)>)>) -> impl IntoView {
    ...
    let new_picks: WriteSignal<bool> = use_context().expect("You should be able to refresh picks");

    let pick_views = initial_values
        .into_iter()
        .map(move |value| match value.0.as_str() {
            "SpreadGroup" => view! {
...
pub fn UserInputs(initial_values: Vec<(Event, Pick)>) -> impl IntoView {
    ...
    let reactive_events: Vec<(Event, RwSignal<Pick>)> =
        initial_values
            .into_iter()
            .map(|(event, pick)| {
                if pick.choice.is_none() {
