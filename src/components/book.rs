...
use crate::{
    ...
    objects::{BookRole, BookSubscription, FrontendUser},
    server::{
        get_book, get_book_table, get_chapters, get_subsciption, AddUser, DeleteBook, DemoteAdmin,
        PromoteAdmin, RemoveUser,
    },
};
...
pub fn VerifiedView() -> impl IntoView {
    ...
    let chapters = create_resource(|| (), move |_| get_chapters(book_id));
...
pub fn UserOptions(
    user: FrontendUser,
    user_subscription: BookSubscription,
    _user_selector: WriteSignal<Option<FrontendUser>>,
) -> impl IntoView {
